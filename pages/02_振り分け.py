        ...
        for number in range(4):
            if search_level(name_list[number])!=0:
                if search_win(name_list[number])==True:
                    ...
                    point_add(name_list[number],ave_level/search_level(name_list[number])+LOSE_POINT)          #ポイントを追加する
        #履歴追加
...
for number in range(4):
    ws_court.cell(row=number+2,column=1*2-1,value=edited_df["勝敗A"][number])        #Aコート
    ws_court.cell(row=number+2,column=2*2-1,value=edited_df["勝敗B"][number])        #Bコート
    ws_court.cell(row=number+2,column=3*2-1,value=edited_df["勝敗C"][number])        #Cコート
    
    #ws_court.cell(row=number+1,column=3*2-1,value=edited_df["勝敗A"][number])
wb.save("data.xlsx")
